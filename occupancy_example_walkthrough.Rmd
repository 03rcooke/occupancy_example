...
# load sparta model
mod_out <- load_rdata("data/processed/Lasius niger_orig.rdata")
...
data_summary <- attributes(mod_out)$metadata$analysis$summary %>% 
  as.data.frame()

# expected validity of model (see Pocock et al., 2019)
spp_metrics <- attributes(mod_out)$metadata$analysis$spp_Metrics %>% 
  as.data.frame()
